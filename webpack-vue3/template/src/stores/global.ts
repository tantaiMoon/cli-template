...
import { RouteLocationNormalizedGeneric, RouteRecordNameGeneric } from 'vue-router'
...
export const useGlobalStore = defineStore(
  ...
  () => {
    ...
    const menuTree = computed(() => filterAsyncRoutes(state.system.menuList))
    ...
    const login = async (user: UserLoginData) => {
      try {
        ...
        state.userInfo = data?.user as IProfile
    ...
    const getPermissions = async () => {
      try {
        ...
        console.log(data)
        state.system.menuList = data.menus as any
        state.system.roles = data.roles as any
        state.system.permissions = data.permissions as string[]
        console.log(filterAsyncRoutes(state.system.menuList))
      } catch {
        //
      }
    }
